(function () {
    ...
    NETWORK.GRAPH.TransformerEdges.prototype.decorateEdges = function (svg, edgesData) {
        ...
        this.wrapperRect = this.gCircle.selectAll(".edgeCentre1").data(this.data).enter().append("rect").attr({
            "class": "tranformerEdgeDecoratorWrapper", "height": 2 * VIEWS.SharedFunctionality.R / 3,
            "width": 4 * VIEWS.SharedFunctionality.R / 3
        });


        this.edgeDecoratorCircle1 = this.gCircle.selectAll(".edgeCentre1").data(this.data).enter().append("circle").attr({
            // 选择画布中所有边的中心点
            "class": "edgeDecorator",
            "r": VIEWS.SharedFunctionality.R / 3
            // 在这些位置各添加一个半径为R/3的圆形
        })
        ...
        this.edgeDecoratorCircle2 = this.gCircle.selectAll(".edgeCentre2").data(this.data).enter().append("circle").attr({
            "class": "edgeDecorator secTransEdgeDecorator",
            "r": VIEWS.SharedFunctionality.R / 3
        })
            .classed("OffStatus", function (d) {
                if (parseInt(d.edgeData.status) === 0) {
