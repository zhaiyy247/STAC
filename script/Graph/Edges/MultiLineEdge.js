(function () {
    ...
    NETWORK.GRAPH.MultiLineEdges.prototype.tick = function () {
        ...
        if (this.standardEdges.edges[0].length !== 0) {
            this.standardEdges.edges.each(function (d) {
                Obj.moveEdges(this);
            });
        ...
        if (this.transformerEdges.edges[0].length !== 0) {
            this.transformerEdges.edges.each(function (d) {
                Obj.moveEdges(this);
            });
        ...
        if (this.lineChargeEdges.edges[0].length) {
            this.lineChargeEdges.edges.each(function (d) {
                Obj.moveEdges(this);
            });
    ...
    NETWORK.GRAPH.MultiLineEdges.prototype.moveDecorator = function () {
        ...
        let color = '';
        conData = this.conPos;
        if (this.standardEdgesData.length !== 0) {
            color = colors.standardEdgeColor;
        } else if (this.transformerEdgesData.length !== 0) {
            color = colors.transformerEdgeColor;
        } else if (this.lineChargeEdgesData.length !== 0) {
            color = colors.lineChargeEdgeColor;
        }
        let lineStyle = `stroke: ${color}`;
        ...
        // 更改multiline辅助线段的颜色
        con1[0].setAttribute("style", lineStyle);
        con2[0].setAttribute("style", lineStyle);
        con3[0].setAttribute("style", lineStyle);
        con4[0].setAttribute("style", lineStyle);
